...
def deep_test():
	sess = tf.Session()
	trainData = np.loadtxt('data/test.csv', delimiter=',', skiprows=1)
	idX = trainData[:, 0:1]
	X = trainData[:, 1:]
	print X.shape
	print idX.shape
	...
	autoencoder = create(x, [200,150 , 50])
	init = tf.initialize_all_variables()
	sess.run(init)
	...
	print 'Number of batched', numofbatches
	outFile = open('features_test.csv', 'a')
	features = []
	for i in range(numofbatches):
		# make a batch of 100:
		batch = []
		if(i < numofbatches - 1):
  			batch = X[i * batchsize: i * batchsize + batchsize, :]
		else:
			batch = X[i * batchsize:, :]
		...
		features.append(sess.run(autoencoder['encoded'], feed_dict={x: batch}))
	print features.size
	for i in range(m):
		outFile.write("{},{}".format(idX[i], features[i]))
	outFile.close()	
